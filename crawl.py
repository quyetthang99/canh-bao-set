...
def crawl_lightning_data():
    # 1. CHIÊU TRÒ CHỐNG CACHE: Gắn thời gian thực vào link để ép Proxy lấy dữ liệu mới
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 2. LƯỚI VÉT SIÊU RỘNG: Tìm tất cả các khối {} (Bất chấp thứ tự, chỉ cần có lat và lng)
            blocks = re.findall(r'\{[^{}]*\}', raw_text)
            
            # Lọc sơ bộ các khối có chứa từ khóa lat, lng và nam (năm)
            ...
            for block in valid_blocks:
                try:
                    ...
                    # Lấy cường độ và loại (Nếu sét ngoài biển không có thì cho mặc định là 0)
                    ...
                    # Giây đôi khi bị thiếu, nếu thiếu cho bằng 0
            ...
            if put_response.status_code == 200:
                print(f"✅ HOÀN TẤT! Đã vét được {diem_moi} điểm sét. Tổng kho: {len(filtered_db)} điểm.")
            else:
