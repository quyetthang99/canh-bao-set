...
def crawl_lightning_data():
    ...
    # Proxy mới: Dùng AllOrigins với lệnh cấm lưu đệm (disableCache)
    ...
    try:
        ...
        # 1. Thử Vượt Rào: Kết nối trực tiếp để chống delay tuyệt đối
        use_proxy = False
        try:
            print("Đang thử kết nối trực tiếp đến trạm Khí tượng...")
        ...
        # 2. Nếu trực tiếp bị chặn, bật Proxy siêu tốc
        if use_proxy:
            print("Kết nối trực tiếp thất bại, tự động chuyển sang Proxy AllOrigins...")
            response = requests.get(proxy_url, headers=headers, timeout=60)

        if response.status_code == 200:
            raw_text = response.text
            print(f"👉 Tải thành công! Độ dài mã nguồn: {len(raw_text)} ký tự.")
            ...
            for block in valid_blocks:
                try:
                    ...
                    # TỰ ĐỘNG SỬA LỖI: Nếu Khí tượng ghi ngược (Kinh độ luôn lớn hơn Vĩ độ ở VN)
                    if lat > lng:
                        lat, lng = lng, lat
            ...
            if put_response.status_code == 200:
                print(f"✅ HOÀN TẤT! Đã lưu {diem_moi} điểm sét mới. Bỏ qua {diem_bi_loai} điểm ngoài lãnh thổ. Tổng kho: {len(filtered_db)} điểm.")
            else:
